...
- late-route mean absolute error remains below a broad regression ceiling;
...
- reduce sensitivity to one unusually slow or fast recent segment;
- account for documented delay without double-counting it;
- weight historical pace by route similarity and sample size;
- measure forecast error by route progress band (25%, 50%, 75%+);
